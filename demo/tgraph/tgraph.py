...
class TGraph(): # Changed here for fp-dash: we can pass the dataframe as parameters instead of the filename
    def __init__(self, df, source=fn.SOURCE, destination=fn.DESTINATION, measure=None, timestamp=None):
        print(source, destination, measure, timestamp)
        
        self.data_network = data_network.DataNetwork(df=df, source=source, destination=destination, measure=measure, timestamp=timestamp)
